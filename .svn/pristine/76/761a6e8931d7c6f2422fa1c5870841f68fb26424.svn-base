...
function addCalendar($st, $et, $sub, $ade){
  ...
  try{
    $db = new DBConnection();
    $db->getConnection();
    $sql = "insert into ".$nam."_calendar (`subject`, `starttime`, `endtime`, `isalldayevent`) values ('"
...
function addDetailedCalendar($st, $et, $sub, $ade, $dscr, $loc, $color, $tz){
  ...
  try{
    $db = new DBConnection();
    $db->getConnection();
    $sql = "insert into ".$nam."_calendar (`subject`, `starttime`, `endtime`, `isalldayevent`, `description`, `location`, `color`) values ('"
      .mysql_real_escape_string($sub)."', '"
      .php2MySqlTime(js2PhpTime($st))."', '"
      .php2MySqlTime(js2PhpTime($et))."', '"
      .mysql_real_escape_string($ade)."', '"
...
function listCalendarByRange($sd, $ed){
  ...
  try{
    $db = new DBConnection();
    $db->getConnection();
    $sql = "select * from ".$nam."_calendar where `starttime` between '"
...
function removeCalendar($id){
  ...
  try{
    $db = new DBConnection();
    $db->getConnection();
    $sql = "delete from rohit_calendar where `id`=" . $id;
		if(mysql_query($sql)==false){
      $ret['IsSuccess'] = false;
      $ret['Msg'] = mysql_error();
    }else{
